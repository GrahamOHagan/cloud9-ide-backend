...
is_intellij_connected() {
    pgrep -u ec2-user -f JetBrains -a | grep -v -F 'remote-dev-server.sh' >/dev/null
}

if is_shutting_down; then
    if [[ ! $SHUTDOWN_TIMEOUT =~ ^[0-9]+$ ]] || is_vfs_connected || is_vscode_connected || is_intellij_connected; then
        sudo shutdown -c
